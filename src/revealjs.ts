...
async function selectReveal(): Promise<string | undefined> {
...
export async function showReveal(doc: vscode.TextEditor) {
    ...
    let qry = vscode.window.activeTextEditor.document.fileName;
      ...
      const updateWebview = async () => {
        if (panel.visible) {
            let agd = await ODb.revealjs(qry);
            panel.title = 'Reveal: ' + qry;
            if (agd["Ok"] === true) {
