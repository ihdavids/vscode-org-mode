...
async function selectImpress(): Promise<string | undefined> {
...
export async function showImpress(doc: vscode.TextEditor) {
    ...
    let qry = vscode.window.activeTextEditor.document.fileName;
      ...
      const updateWebview = async () => {
        if (panel.visible) {
            let agd = await ODb.impressjs(qry);
            panel.title = 'Impress: ' + qry;
            if (agd["Ok"] === true) {
    ...
    panel.webview.onDidReceiveMessage(message => {
      ...
      panel.onDidDispose(
        () => {
          ...
          console.log("CLOSE IMPRESS");
        },
        null,
        OrgExtension.get().context.subscriptions
      );

    console.log("SHOW IMPRESS SHOWN");
}